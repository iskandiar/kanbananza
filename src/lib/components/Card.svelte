...
<div class="group rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 cursor-grab active:cursor-grabbing hover:border-indigo-500/40 transition-colors">
  {#if meetingTime()}
  ...
  <div class="mt-1.5 flex items-center gap-1.5 flex-wrap">
    ...
    {/if}
    {#if card.url}
      <a href={card.url} target="_blank" class="text-xs text-[var(--color-muted)] hover:text-indigo-400 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" onclick={(e) => e.stopPropagation()}>↗</a>
    {/if}
  </div>
</div>
