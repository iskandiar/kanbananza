<script lang="ts">
  ...
  onMount(async () => {
    connected = (await getSecret('kanbananza', 'gitlab_pat')) !== null;

    unlistenSynced = await listen<{ count: number; error: string | null }>('gitlab://synced', (event) => {
      syncing = false;
      if (event.payload.error) {
        ...
        error = null;
        syncMessage = `Synced ${event.payload.count} MR${event.payload.count === 1 ? '' : 's'}`;
      }
    });
  });
  ...
  async function savePat() {
    ...
    try {
      await storeSecret('kanbananza', 'gitlab_pat', patInput.trim());
      connected = true;
      editing = false;
      patInput = '';
      saved = true;
      ...
      error = `Failed to save PAT: ${String(e)}`;
    }
  }
  ...
  {#if editing}
    <div class="flex items-center gap-2 px-0 py-2 border-b border-[var(--color-border)]">
      <input
        ...
        placeholder="glpat-…"
        class="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded px-3 py-1.5 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] focus:outline-none focus:ring-1 focus:ring-indigo-500"
        onkeydown={(e) => { if (e.key === 'Enter') savePat(); if (e.key === 'Escape') { editing = false; patInput = ''; } }}
      />
      <button
        onclick={savePat}
        disabled={!patInput.trim()}
      ...
      <button
        onclick={() => { editing = false; patInput = ''; }}
        class="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors"
      >
        Cancel
      </button>
